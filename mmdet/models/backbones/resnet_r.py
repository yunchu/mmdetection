...
class RobustResNetWrapper(nn.Module):
    def __init__(self, output_features, depth=18):
        super(RobustResNetWrapper, self).__init__()
        if depth == 18:
            ...
            raise ValueError('Wrong resnet depth')
    ...
    def forward(self, x):
        ...
        if 1 in self.output_features:
            outs.append(x)
        x = self.model.layer2(x)
        if 2 in self.output_features:
            outs.append(x)
        x = self.model.layer3(x)
        if 3 in self.output_features:
            outs.append(x)
        x = self.model.layer4(x, fake_relu=False, no_relu=False)
        if 4 in self.output_features:
            outs.append(x)
